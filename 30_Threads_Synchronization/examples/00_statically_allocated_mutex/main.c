 ...
 *              with mutexes to synchronize access to a shared resource.
...
void *pthread_func(void *num) {
    ...
    for (int i = 0; i < 10000; i++) {
         ...
         * Returns:
         * - 0 if successful.
         * - Error code if mutex could not be locked.
         */
        status = pthread_mutex_lock(&mtx);
        if (status != 0) {
            printf("Thread [%d] can't lock the mutex\n", (int)num);
            pthread_exit(NULL);  /* Exit the thread if locking fails */
        }
